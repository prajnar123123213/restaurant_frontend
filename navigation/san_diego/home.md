---
layout: post
title: San Diego 
search_exclude: true
permalink: /san_diego/home
menu: nav/san_diego.html
---

<!DOCTYPE html>
<html>
<head>
<style>
body {
  background: linear-gradient(to bottom, #FFC0CB, #FF69B4); /* Pink gradient background */
  background-attachment: fixed;
  margin: 0;
  padding: 0;
  font-family: 'Comic Sans MS', 'Brush Script MT', cursive;
}
.myDiv {
  border: 5px solid white;
  background: linear-gradient(to bottom right, #FFB6C1, #FF69B4); /* Pink gradient background for the container */
  text-align: center;
  padding: 30px;
  border-radius: 15px; /* Rounded corners */
  box-shadow: 0 10px 20px rgba(0, 0, 0, 0.3); /* Shadow effect */
  animation: fadeIn 1.5s ease-in-out;
  max-width: 80%;
  margin: 40px auto;
  transition: transform 0.3s ease, box-shadow 0.3s ease; /* Smooth transition for hover effect */
}
.myDiv:hover {
  transform: scale(1.03); /* Slightly enlarges the box on hover */
  box-shadow: 0 15px 30px rgba(0, 0, 0, 0.4); /* Pronounced shadow effect */
}
img {
  border: 5px solid #FFC0CB; /* Pink border around images */
  transition: transform 0.4s ease; /* Smooth zoom effect */
}
img:hover {
  transform: scale(1.05); /* Slight zoom effect on hover */
}
.button {
  background-color: #FF69B4; /* Hot pink button background */
  color: white;
  border: none;
  padding: 10px 20px;
  text-align: center;
  text-decoration: none;
  display: inline-block;
  font-size: 16px;
  font-family: 'Comic Sans MS', 'Brush Script MT', cursive;
  border-radius: 5px;
  cursor: pointer;
  transition: all 0.3s ease; /* Smooth transition for hover effect */
  box-shadow: 0 4px 6px rgba(0, 0, 0, 0.2);
}
.button:hover {
  background-color: #FFB6C1; /* Lighter pink hover effect */
  color: white; /* Keeps text white on hover */
  transform: translateY(-5px); /* Lifts the button slightly on hover */
  box-shadow: 0 8px 12px rgba(0, 0, 0, 0.3); /* Enhanced shadow */
}
@keyframes fadeIn {
  from {
    opacity: 0;
    transform: translateY(20px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}
</style>
</head>
<body>



<style>
    body {
        background: linear-gradient(to bottom,rgb(133, 145, 255),rgb(52, 60, 177)) !important; /* Orange to Yellow gradient */
        background-attachment: fixed !important;
        margin: 0 !important;
        padding: 0 !important;
    }

    .centered-text {
        text-align: center;
        color:rgb(255, 255, 255) !important; /* Light yellow text */
        font-family: 'Comic Sans MS', 'Brush Script MT', cursive !important; /* Fun and playful font */
        font-size: 2.5em !important; /* Adjusted size for emphasis */
        margin-top: 20px !important;
    }
</style>

<div class="centered-text">
    <h1>Welcome to San Diego!</h1>
</div>
<br>
<center>
    <img src="{{site.baseurl}}/images/sandiego.jpg" alt="San Diego" width="500" height="500">
</center>

<link rel="stylesheet" href="styles.css">
